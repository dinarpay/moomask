                ...
                <div>
                    <external-link :url="getTransactionLink(transaction.hash)" class="transaction" v-for="transaction in transactions" :key="transaction.hash">
                        <span class="transaction-content">
                            <span class="transaction-contract">{{ transaction.value }}</span>
                            <span class="transaction-date">{{ formatStamp(transaction.timeStamp, { day: 'numeric', month: 'long', year: 'numeric' }) }}</span>
                        </span>
...
<script>
    ...
    import { CONTRACT_TYPES, getScanlink } from '../../lib/utils' 
    import { decryptKeyStore } from '../../lib/keystore'
    import AppHeader from '../components/AppHeader.vue'
    import ExternalLink from '../components/ExternalLink.vue'
    import getTransactionsByAccount from '../../lib/transactions'
    ...
    export default {
        ...
        methods: {
            async loadTransactions() {
                ...
                const transactionsData = await getTransactionsByAccount(this.network, this.address);

                this.$store.commit('account/transactions', transactionsData)
            ...
            getTransactionLink(hash) {
                return `${getScanlink(hash)}tx/${hash}`;
            },

            formatStamp(stamp, opts) {
                const dt = new Date(stamp * 1000)
